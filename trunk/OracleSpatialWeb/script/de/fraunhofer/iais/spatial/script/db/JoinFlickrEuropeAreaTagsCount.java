...
//			countTotal(conn);
	...
	private void countHoursTags(Connection conn, String radiusString) throws SQLException {
		...
		while (selectAreaRs.next()) {
			...
			if(hoursTagsCount.size() > 0){
				addToIndex(conn, hoursTagsCount, Level.HOUR, areaId, radiusString);
			}
	...
	private void addToIndex(Connection conn, Map<String, Map<String, Integer>> countsMapToAdd, Level queryLevel, int id, String radius) throws SQLException {
	...
	private void countDay(Connection conn) throws SQLException {
		...
		while (pset.next()) {
			...
			if (hourStr != null) {
				...
				addToIndex(conn, daysTagsCount, Level.DAY, id, radius);
	...
	private void countMonth(Connection conn) throws SQLException {
		...
		while (pset.next()) {
			...
			if (hourStr != null) {
				...
				addToIndex(conn, monthsTagsCount, Level.MONTH, id, radius);
	...
	private void countYear(Connection conn) throws SQLException {
		...
		while (pset.next()) {
			...
			if (hourStr != null) {
				...
				addToIndex(conn, zearsTagsCount, Level.YEAR, id, radius);
