...
public abstract class AbstractKernelTest extends TestCase {
	...
	protected CheckOptionHandler getOptionTester() {
		...
		if (getKernel() instanceof OptionHandler) {
			result.setOptionHandler(getKernel());
		} else {
	...
	public void testRegression() throws Exception {
		...
		for (i = FIRST_CLASSTYPE; i <= LAST_CLASSTYPE; i++) {
			...
			train = m_Tester.makeTestDataset(42, m_Tester.getNumInstances(), m_NominalPredictors[i] ? 2 : 0, m_NumericPredictors[i] ? 1 : 0, m_StringPredictors[i] ? 1 : 0, m_DatePredictors[i] ? 1 : 0, m_RelationalPredictors[i] ? 1 : 0, 2, i,
					m_multiInstanceHandler);

			try {
				m_RegressionResults[i] = CheckScheme.attributeTypeToString(i) + " class:\n" + useKernel(train);
				succeeded = true;
