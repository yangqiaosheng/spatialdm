...
public class ThresholdSelectorTest extends AbstractClassifierTest {
		...
		do {
			try {
				...
				if ((msg.indexOf("string") != -1) && (msg.indexOf("attributes") != -1)) {
					...
					tag = new SelectedTag(Attribute.NOMINAL, RemoveType.TAGS_ATTRIBUTETYPE);
					invert = true;
				} else if ((msg.indexOf("only") != -1) && (msg.indexOf("numeric") != -1)) {
					System.err.println("\nDeleting non-numeric attributes.");
					tag = new SelectedTag(Attribute.NUMERIC, RemoveType.TAGS_ATTRIBUTETYPE);
					invert = true;
				} else
					throw ex;
	...
	public void testDesignatedClass() throws Exception {
		...
		for (Tag element : ThresholdSelector.TAGS_OPTIMIZE) {
			((ThresholdSelector) m_Classifier).setDesignatedClass(new SelectedTag(element.getID(), ThresholdSelector.TAGS_OPTIMIZE));
			m_Instances.setClassIndex(1);
	...
	public void testEvaluationMode() throws Exception {
		...
		for (Tag element : ThresholdSelector.TAGS_EVAL) {
			((ThresholdSelector) m_Classifier).setEvaluationMode(new SelectedTag(element.getID(), ThresholdSelector.TAGS_EVAL));
			m_Instances.setClassIndex(1);
			FastVector result = useClassifier();
			assertTrue(result.size() != 0);
		}
