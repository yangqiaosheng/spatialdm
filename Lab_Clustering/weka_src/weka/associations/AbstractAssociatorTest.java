...
public abstract class AbstractAssociatorTest extends TestCase {
	...
	protected String getClassTypeString(int type) {
		if (type == LAST_CLASSTYPE + 1)
			...
			return CheckScheme.attributeTypeToString(type);
	}
	...
	public void testNClasses() {
		...
		result = m_Tester.canHandleNClasses(m_NominalPredictors[Attribute.NOMINAL], m_NumericPredictors[Attribute.NOMINAL], m_StringPredictors[Attribute.NOMINAL], m_DatePredictors[Attribute.NOMINAL], m_RelationalPredictors[Attribute.NOMINAL],
				m_multiInstanceHandler, m_NClasses);
	...
	public void testDatasetIntegrity() {
		...
		for (i = FIRST_CLASSTYPE; i <= LAST_CLASSTYPE + 1; i++) {
			...
			result = m_Tester
					.datasetIntegrity(m_NominalPredictors[i], m_NumericPredictors[i], m_StringPredictors[i], m_DatePredictors[i], m_RelationalPredictors[i], m_multiInstanceHandler, type, m_handleMissingPredictors[i], m_handleMissingClass[i]);
	...
	public void testRegression() throws Exception {
		...
		for (i = FIRST_CLASSTYPE; i <= LAST_CLASSTYPE + 1; i++) {
			...
			train = m_Tester.makeTestDataset(42, m_Tester.getNumInstances(), m_NominalPredictors[i] ? 2 : 0, m_NumericPredictors[i] ? 1 : 0, m_StringPredictors[i] ? 1 : 0, m_DatePredictors[i] ? 1 : 0, m_RelationalPredictors[i] ? 1 : 0, 2, type,
