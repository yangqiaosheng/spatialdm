...
/**
* A component supporting the "visual comparison" operation applied to
* cartographic representation of a single numeric attribute. The component
* allows the user to set a reference value, and the map changes so as to
...
public class VisComparison extends Panel implements SliderListener, FocusListener, ActionListener, PropertyChangeListener, Manipulator, DataTreater, EventReceiver, EventSource, ObjectEventHandler, ObjectEventReactor, Destroyable {
	/**
	* Used to generate unique identifiers of instances of VisComparison
	*/
	...
	*/
	...
	public boolean construct(Supervisor sup, Object visualizer, AttributeDataPortion dataTable) {
		...
		pp.add(new Label(vis.getAttrName(0)));
		pp.add(p);
		add(pp, "North");
