 ...
 */
public class SpatialDistanceTrajectories_EndsAndDistSteps extends SpatialDistance implements TimeTransformationUser {
	...
	public boolean askParameters() {
		...
		p.add(iip = new InputIntPanel("Distance step (in m)?", 1000, 1, 10000000, "Method of computing distances: " + methodName));
		...
		step = Dialogs.askForIntValue(CManager.getAnyFrame(),
		      "Distance step (in m)?",1000,1,10000000,
	...
	public String getParameterDescription() {
		return "Distance step = " + step + " meters; adjustment: " + ((adjustStart) ? "starts" : "ends");
	}
	...
	public double distance(TrajectoryObject obj1, TrajectoryObject obj2, boolean useThreshold) {
		...
		if (useThreshold && (dsum > 2 * distanceThreshold))
			return Double.POSITIVE_INFINITY;
		int idx1[] = obj1.getAbsDistIdx(step, adjustStart), idx2[] = obj2.getAbsDistIdx(step, adjustStart);
