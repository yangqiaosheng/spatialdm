 ...
 * 
 */
public class SpatialListIndexDatabase implements Database, Serializable, RevisionHandler {

	TreeMap map = new TreeMap();
	// two dimensional kd-tree
	KDTree<EventList> tree = new KDTree<EventList>(2);
	Instances instances = null;
	int id = 0;

	public SpatialListIndexDatabase(Instances instances) {
	...
	public List epsilonRangeQuery(double epsilon, DataObject queryDataObject) {
		...
		try {
			...
			List<EventList> list = tree.nearestGeo(coordinates, epsilon);//tree.range(lowk, uppk);
	...
	public void insert(DataObject dataObject) {
		// TODO Auto-generated method stub
		id++;
		Event obj = (Event) dataObject;

		try {
			double[] coordinates = { obj.getInstance().valueSparse(0), obj.getInstance().valueSparse(1) };
			map.put(dataObject.getKey(), dataObject);
			EventList res = tree.search(coordinates);
			if (res == null) {
				res = new EventList(obj.getInstance(), obj.getKey(), this);
				res.add(obj);
