 ...
 */
public class HullBuilder extends GeoCalculator implements spade.lib.util.Comparator {
	...
	public Object doCalculation(LayerManager lman, ESDACore core) {
		...
		SelectDialog selDia = new SelectDialog(core.getUI().getMainFrame(), "Hulls or buffers?", "What should be built?");
		selDia.addOption("convex hulls", "hulls", false);
		selDia.addSeparator();
		selDia.addOption("buffers", "buffers", true);
		Panel p = new Panel(new BorderLayout());
		p.add(new Label("Buffer width:"), BorderLayout.WEST);
		if (geo) {
			p.add(new Label("m"), BorderLayout.EAST);
		}
		TextField tf = new TextField("0", 10);
		p.add(tf, BorderLayout.CENTER);
		selDia.addComponent(p);
		selDia.addSeparator();
		selDia.addLabel("Layer extent = " + StringUtil.doubleToStr(extent, precDist) + ((geo) ? " m" : ""));
		boolean buildBuffers = false;
		double bufDistOrig = Double.NaN;
		do {
			selDia.show();
			if (selDia.wasCancelled())
				return null;
			buildBuffers = selDia.getSelectedOptionN() == 1;
			if (buildBuffers) {
				...
				if (Double.isNaN(bufDistOrig)) {
					showMessage("The buffer width is not properly specified!", true);
				}
			}
		} while (buildBuffers && Double.isNaN(bufDistOrig));
		...
		//The GeometryFactory will be used to construct JTS multipoint geometries
		...
		for (int i = 0; i < groups.size(); i++) {
			...
			Coordinate coords[] = new Coordinate[grSize];
			for (int j = 0; j < grSize; j++) {
				RealPoint rp = gr.getPoint(j);
				coords[j] = new Coordinate(rp.x, rp.y);
			}
			MultiPoint mpt = geoFactory.createMultiPoint(coords);
			com.vividsolutions.jts.geom.Geometry hull = null;
			try {
				if (buildBuffers) {
					hull = mpt.buffer(bufDist);
				} else {
					hull = mpt.convexHull();
				}
			} catch (Exception e) {
			...
			RealPoint pt = gr.getRepresentativePoint();
			rec.setNumericAttrValue(pt.x, StringUtil.floatToStr(pt.x, precCoord), 6);
			rec.setNumericAttrValue(pt.y, StringUtil.floatToStr(pt.y, precCoord), 7);
			gr.computeMedian();
		...
		name = ((buildBuffers) ? "Buffers (" + bufDistOrig + ")" : "Convex hulls") + " around groups of " + pointLayer.getName();
		DGeoLayer hullLayer = new DGeoLayer();
		hullLayer.setName(name);
		hullLayer.setType(Geometry.area);
		hullLayer.setGeographic(pointLayer.isGeographic());
		hullLayer.setGeoObjects(pObj, true);
		hullLayer.setDataTable(table);
