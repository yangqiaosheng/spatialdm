...
/**
* Makes a combination of several raster layers using various functions and
...
public class RasterCombiner extends GeoCalculator {
	...
	/**
	* Combiner several raster layers. The arguments are a layer manager (a
	...
	*/
	...
	public Object doCalculation(LayerManager lman, ESDACore core) {
		...
		EnterFormula enterFormula = new EnterFormula(lnames);
		Frame win = null;
		if (ui != null) {
			win = ui.getMainFrame();
		}
		...
		OKDialog okd = new OKDialog(win, res.getString("Specify_the_formula1"), true);
		...
		float cDX = Float.MAX_VALUE, cDY = Float.MAX_VALUE;
...
//      if (cDY < crg.DY) cDY=crg.DY;
		...
		if (!sameBounds) {
			...
			Checkbox cbInter = new Checkbox(res.getString("intersection"), true, cbg), cbUnion = new Checkbox(res.getString("union"), false, cbg);
			pp.add(cbInter);
			pp.add(cbUnion);
			p.add(pp, "Center");
...
//            (crg.getInterpolatedValue(crg.getGridX(comb.getWorldX(xx)), crg.getGridY(comb.getWorldY(yy))))
				...
				comb.ras[xx][yy] = (float) ca.useTrack();
				if (Float.isInfinite(comb.ras[xx][yy])) {
		...
		comb.recalculateStatistics();
		String name = enterFormula.getFormula();
		for (int i = 0; i < inUse.length; i++) {
			GeoLayer gl = lman.getGeoLayer(lidx[i]);
		...
		return constructRasterLayer(comb, name);
	}
}