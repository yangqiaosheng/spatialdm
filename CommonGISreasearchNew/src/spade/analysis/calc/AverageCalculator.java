...
/**
* Calculates average among given table columns (specified in fn). Returns a
* vector with the identifier of the new attribute added to the table.
*/
public class AverageCalculator extends BaseCalculator {
	static ResourceBundle res = Language.getTextResource("spade.analysis.calc.Res");
	// following text:"Computes the arithmetic mean (average) of / values of two or more attributes in each row of the table."
	public static final String expl = res.getString("Computes_the") + res.getString("values_of_two_or_more");

	// following text: "Select at least two numeric attributes / for the computation of the arithmetic mean (average) of their values "
	public static final String prompt = res.getString("Select_at_least_two") + res.getString("for_average");
	...
	*/
	...
	public Vector doCalculation() {
		...
		String name = res.getString("Average") + " ";
		if (attrDescr != null && attrDescr.size() == 1) {
			AttrDescriptor ad = (AttrDescriptor) attrDescr.elementAt(0);
			name += ad.getName();
		} else {
			name += res.getString("from") + " ";
		...
		double sum[] = TableStat.getMeanOfColumns(dTable, fn);
		// adding the column to the table
		int idx = dTable.addDerivedAttribute(name, AttributeTypes.real, AttributeTypes.average, sourceAttr);
	...
	*/
	protected Vector doParameterDependentCalculation() {
		...
		String name = res.getString("Average") + " " + res.getString("from") + " " + ad.attr.getName();
		...
		for (int i = 0; i < ad.children.size(); i++) {
			...
			int attrIdx = dTable.addDerivedAttribute(name, AttributeTypes.real, AttributeTypes.average, sourceAttr);
			double results[] = TableStat.getMeanOfColumns(dTable, cn);
