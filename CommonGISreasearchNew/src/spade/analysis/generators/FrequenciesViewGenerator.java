...
import spade.analysis.plot.ValueFrequencyViewer;
 ...
 * Date: 18.01.2007
 * Time: 18:00:23
 * Generates instanced of ValueFrequencyViewer
 */
public class FrequenciesViewGenerator extends VisGenerator {
	...
	*/
	...
	public Component constructDisplay(String methodId, Supervisor sup, AttributeDataPortion dataTable, Vector attributes, Hashtable properties) {
		if (!isApplicable(dataTable, attributes))
			return null;
		int idx = dataTable.getAttrIndex((String) attributes.elementAt(0));
		ValueFrequencyViewer viewer = new ValueFrequencyViewer((DataTable) dataTable, idx, sup);
		return viewer;
	}
}
