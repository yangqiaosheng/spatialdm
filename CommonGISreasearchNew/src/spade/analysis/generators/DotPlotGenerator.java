...
public abstract class DotPlotGenerator extends VisGenerator {
	...
	*/
	public DotPlot constructDotPlot(String methodId, Supervisor sup, AttributeDataPortion dataTable, Vector attributes, boolean isHorisontal, Hashtable properties) {
		if (!isApplicable(dataTable, attributes))
			return null;
		DotPlot dp = new DotPlot(isHorisontal, true, true, sup, sup);
		dp.setMethodId(methodId);
		dp.setDataSource(dataTable);
		dp.setFieldNumber(dataTable.getAttrIndex((String) attributes.elementAt(0)));
