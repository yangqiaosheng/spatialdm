...
/**
* Gets a raster from a FLT file
*/
public class FLTReader extends DataStreamReader implements GeoDataReader, DataSupplier {
	...
	public boolean loadData(boolean mayAskUser) {
		...
		if (spec == null || spec.source == null)
			if (mayAskUser) {
				...
				String path = browseForFile(res.getString("Select_the_file_with"), "*.flt");
				if (path == null) {
	...
	*/
	protected LayerData readSpecific() {
		...
		try {
			...
			DY = (new Float(st.nextToken())).floatValue();
			Intr = st.nextToken().compareTo("1") == 0 ? true : false;
			Geog = st.nextToken().compareTo("1") == 0 ? true : false;
