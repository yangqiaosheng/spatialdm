...
public class ClassifierPanel extends JPanel {
	...
	static {
		...
		java.beans.PropertyEditorManager.registerEditor(weka.classifiers.Classifier[].class, weka.gui.GenericArrayEditor.class);
		java.beans.PropertyEditorManager.registerEditor(DistributionClassifier.class, weka.gui.GenericObjectEditor.class);
		java.beans.PropertyEditorManager.registerEditor(weka.classifiers.Classifier.class, weka.gui.GenericObjectEditor.class);
		java.beans.PropertyEditorManager.registerEditor(weka.classifiers.CostMatrix.class, weka.gui.CostMatrixEditor.class);
	}
	...
	public ClassifierPanel() {
		...
		m_MoreOptions.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				final javax.swing.JFrame jf = new javax.swing.JFrame("Classifier evaluation options");
		...
		if (m_Preprocess != null && m_TestInstances != null) {
			...
			if (sf != null) {
				...
				try {
					...
					JOptionPane.showMessageDialog(this, "Could not deserialize filters", null, JOptionPane.ERROR_MESSAGE);
				}
				if (filters.length != 0) {
					try {
						...
						for (int i = 0; i < filters.length; i++) {
							...
							m_TestInstancesCopy = Filter.useFilter(m_TestInstancesCopy, filters[i]);
						}
						m_Log.statusMessage("OK");
					} catch (Exception ex) {
						m_Log.statusMessage("See error log");
						m_Log.logMessage("Problem applying filters to test data " + "(Classifier Panel)");
					}
	...
	private void processClassifierPrediction(Instance toPredict, Classifier classifier, Evaluation eval, FastVector predictions, Instances plotInstances, FastVector plotShape, FastVector plotSize) {
		try {
			...
			if (predictions != null) {
				DistributionClassifier dc = (DistributionClassifier) classifier;
				double[] dist = dc.distributionForInstance(toPredict);
				pred = eval.evaluateModelOnce(dist, toPredict);
				int actual = (int) toPredict.classValue();
				predictions.addElement(new NominalPrediction(actual, dist, toPredict.weight()));
			} else {
				pred = eval.evaluateModelOnce(classifier, toPredict);
			}
			...
			m_RunThread = new Thread() {
				public void run() {
					...
					try {
						...
						if (inst.classAttribute().isNominal() && classifier instanceof DistributionClassifier) {
						...
						case 3: // Test on training
							...
							for (int jj = 0; jj < inst.numInstances(); jj++) {
								processClassifierPrediction(inst.instance(jj), classifier, eval, predictions, predInstances, plotShape, plotSize);
						...
						case 1: // CV mode
							...
							for (int fold = 0; fold < numFolds; fold++) {
								...
								for (int jj = 0; jj < test.numInstances(); jj++) {
									processClassifierPrediction(test.instance(jj), classifier, eval, predictions, predInstances, plotShape, plotSize);
								}
							}
							if (inst.attribute(classIndex).isNominal()) {
						...
						case 4: // Test on user split
							...
							for (int jj = 0; jj < userTest.numInstances(); jj++) {
								processClassifierPrediction(userTest.instance(jj), classifier, eval, predictions, predInstances, plotShape, plotSize);
								if ((jj % 100) == 0) {
						...
						try {
							if (predInstances != null && predInstances.numInstances() > 0) {
								...
								m_CurrentVis.addActionListener(new ActionListener() {
									public void actionPerformed(ActionEvent e) {
										if (m_CurrentVis.getInstances().relationName().compareTo(m_Instances.relationName()) == 0) {
											setXY_VisualizeIndexes(m_CurrentVis.getXIndex(), m_CurrentVis.getYIndex());
										}
									}
								});
	...
	public static void main(String[] args) {
		...
		try {
			final javax.swing.JFrame jf = new javax.swing.JFrame("Weka Knowledge Explorer: Classifier");
			jf.getContentPane().setLayout(new BorderLayout());
