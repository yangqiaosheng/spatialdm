...
public class ClustererPanel extends JPanel {
	...
	private Dimension COMBO_SIZE = new Dimension(250, m_StartBut.getPreferredSize().height);
	...
	static {
		...
		java.beans.PropertyEditorManager.registerEditor(weka.clusterers.Clusterer.class, weka.gui.GenericObjectEditor.class);
	}
			...
			if (sf != null) {
				...
				if (filters.length != 0) {
					try {
						...
						m_Log.logMessage("Problem applying filters to test data " + "(Cluster Panel");
					}
	 ...
	 */
	private PlotData2D setUpVisualizableInstances(Instances testInstances, ClusterEvaluation eval) throws Exception {
		...
		newInsts = new Instances(testInstances.relationName() + "_clustered", hv, testInstances.numInstances());
	...
	public static void main(String[] args) {
		...
		try {
			final javax.swing.JFrame jf = new javax.swing.JFrame("Weka Knowledge Explorer: Cluster");
			jf.getContentPane().setLayout(new BorderLayout());
