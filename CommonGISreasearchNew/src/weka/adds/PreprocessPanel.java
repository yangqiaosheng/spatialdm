...
public class PreprocessPanel extends JPanel {
	...
	static {
		...
		java.beans.PropertyEditorManager.registerEditor(weka.core.SelectedTag.class, weka.gui.SelectedTagEditor.class);
		java.beans.PropertyEditorManager.registerEditor(weka.filters.Filter.class, weka.gui.GenericObjectEditor.class);
		java.beans.PropertyEditorManager.registerEditor(weka.attributeSelection.ASSearch.class, weka.gui.GenericObjectEditor.class);
		java.beans.PropertyEditorManager.registerEditor(weka.attributeSelection.ASEvaluation.class, weka.gui.GenericObjectEditor.class);
		java.beans.PropertyEditorManager.registerEditor(weka.experiment.InstanceQuery.class, weka.gui.GenericObjectEditor.class);
		java.beans.PropertyEditorManager.registerEditor(weka.core.converters.Loader.class, weka.gui.GenericObjectEditor.class);
	}
	...
	public PreprocessPanel() {
		...
		try {
			...
			((GenericObjectEditor.GOEPanel) m_DatabaseQueryEditor.getCustomEditor()).addOkListener(new ActionListener() {
		...
		m_ReplaceBut.setToolTipText("Replace the base relation with the working relation");
		...
		m_BaseInstPanel.setBorder(BorderFactory.createTitledBorder("Base relation"));
		m_WorkingInstPanel.setBorder(BorderFactory.createTitledBorder("Working relation"));
		m_AttPanel.setBorder(BorderFactory.createTitledBorder("Attributes in base relation"));
		m_Filters.setBorder(BorderFactory.createTitledBorder("Filters"));
		m_AttSummaryPanel.setBorder(BorderFactory.createTitledBorder("Attribute info for base relation"));
		...
		try {
			Runnable r = new Runnable() {
				public void run() {
					...
					m_Log.logMessage("Base relation is now " + m_BaseInstances.relationName() + " (" + m_BaseInstances.numInstances() + " instances)");
	...
	public void setWorkingInstances(Instances inst) {
		...
		if (m_WorkingInstances != inst) {
			...
			m_Log.logMessage("Working relation is now " + m_WorkingInstances.relationName() + " (" + m_WorkingInstances.numInstances() + " instances)");
			m_Log.statusMessage("OK");
	...
	protected Instances filterInstances(Instances instances) {
		...
		try {
			...
			for (int i = 0; i < filters.length; i++) {
				m_Log.statusMessage("Passing through filter " + (i + 1) + ": " + filters[i].getClass().getName());
			...
			try {
				...
				JOptionPane.showMessageDialog(this, "Could not create copy of filters", null, JOptionPane.ERROR_MESSAGE);
			}
			return temp;
		} catch (UnassignedClassException ex) {
			// Pop up an error optionpane
			JOptionPane.showMessageDialog(this, "Problem filtering instances:\n" + ex.getMessage() + "\n\n" + "You should not use filters that require " + "a class attribute during preprocessing,\n" + "as this will bias any classification "
					+ "results you obtain. Consider using a\n" + "FilteredClassifier instead.", "Instance Filter", JOptionPane.ERROR_MESSAGE);
			return null;
		} catch (Exception ex) {
			// Pop up an error optionpane
			JOptionPane.showMessageDialog(this, "Problem filtering instances:\n" + ex.getMessage(), "Instance Filter", JOptionPane.ERROR_MESSAGE);
			ex.printStackTrace();
			m_Log.logMessage(ex.getMessage());
			return null;
		}
	...
	public void setWorkingInstancesFromFilters() {
		if (m_IOThread == null) {
			...
			if (m_AttPanel.getSelectedAttributes().length == 0) {
				...
				res = JOptionPane
						.showOptionDialog(null, "Unselecting all attributes will result " + "in an empty dataset.\nDo you want to " + "continue?", "Warning", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, options, options[1]);
			...
			JOptionPane.showMessageDialog(this, "Can't apply filters at this time,\n" + "currently busy with other IO", "Apply filters", JOptionPane.WARNING_MESSAGE);
	...
	public void setBaseInstancesFromDBQ() {
		if (m_IOThread == null) {
			try {
				InstanceQuery InstQ = (InstanceQuery) m_DatabaseQueryEditor.getValue();
			...
			m_IOThread = new Thread() {
				public void run() {
					try {
						...
						m_Log.statusMessage("Problem reading " + f.getName() + " as an arff file.");
						m_IOThread = null;
						converterQuery(f);
