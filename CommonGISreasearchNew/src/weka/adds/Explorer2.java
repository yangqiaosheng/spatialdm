...
public class Explorer2 extends Explorer {
	...
	/** Label for a panel that still need to be implemented */
	protected VisualizePanel m_VisualizePanel = new VisualizePanel();
	...
	public Explorer2() {
		...
		m_LogPanel.logMessage("(c) 1999 The University of Waikato, Hamilton," + " New Zealand");
		...
		m_TabbedPane.addTab("Preprocess", null, m_PreprocessPanel, "Open/Edit/Save instances");
		m_TabbedPane.addTab("Classify", null, m_ClassifierPanel, "Classify instances");
		m_TabbedPane.addTab("Cluster", null, m_ClustererPanel, "Identify instance clusters");
		m_TabbedPane.addTab("Associate", null, m_AssociationPanel, "Discover association rules");
		m_TabbedPane.addTab("Select attributes", null, m_AttributeSelectionPanel, "Determine relevance of attributes");
		m_TabbedPane.addTab("Visualize", null, m_VisualizePanel, "Explore the data");
		...
		m_PreprocessPanel.addPropertyChangeListener(new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent e) {
				m_ClassifierPanel.setInstances(m_PreprocessPanel.getWorkingInstances());
				m_ClassifierPanel.filterUserTestInstances();

				m_AssociationPanel.setInstances(m_PreprocessPanel.getWorkingInstances());
				m_ClustererPanel.setInstances(m_PreprocessPanel.getWorkingInstances());
				m_ClustererPanel.filterUserTestInstances();

				m_AttributeSelectionPanel.setInstances(m_PreprocessPanel.getWorkingInstances());
				try {
					PlotData2D tempd = new PlotData2D(m_PreprocessPanel.getWorkingInstances());
					tempd.setPlotName(m_PreprocessPanel.getWorkingInstances().relationName());
	...
	public static void main(String[] args) {
		...
		try {
			...
			jf.setSize(800, 600);
			jf.setVisible(true);
			Image icon = Toolkit.getDefaultToolkit().getImage(ClassLoader.getSystemResource("weka/gui/weka_icon.gif"));
	...
	public static void main(Instances ins) {
		...
		try {
			...
			if (ins != null) {
				//System.err.println("Loading instances from " + args[0]);
				//explorer.m_PreprocessPanel.setBaseInstancesFromFile(new File(args[0]));
				explorer.m_PreprocessPanel.setBaseInstances(ins);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			System.err.println(ex.getMessage());
