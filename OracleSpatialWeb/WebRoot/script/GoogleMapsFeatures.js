...
function attachMessage(Polygon, nrPolOntheScreen, idp) {
	...
	if (readyToExecute==true){	    
	      ...
	      timeOut = setTimeout(function innerFunction(){   
				...
				//console.log(idp+", "+numberOfTags+", "+centerP+", "+totalP+", "+selectedPolygons+", "+nr);
...
function loadTags(idp, xml, center, total, selc, mouseString){    
	...
	$("#tag").html("");
	for (var i=0;i<contorK;i++){
	      $("#tag").append("<span  id = '"+tags[eval(shuffledVector[i])].getNameTag()+"' onclick='wordClick("+tags[eval(shuffledVector[i])].getNameTag()+")' OnMouseOver='mouseOverTag()'  ><font size="+tags[eval(shuffledVector[i])].getFontSize()+" face='arial' color='blue'>"+tags[eval(shuffledVector[i])].getNameTag()+" </font></span>");
	}
	...
	if ($("#tag").html()==""){
	  for (var i=0;i<contorK;i++){
		$("#tagClick").append("<span id = '"+tags[eval(shuffledVector[i])].getNameTag()+"' onclick='wordClick("+tags[eval(shuffledVector[i])].getNameTag()+")'   OnMouseOver='mouseOverTag()'   ><font size="+tags[eval(shuffledVector[i])].getFontSize()+" face='arial' color='blue'>"+tags[eval(shuffledVector[i])].getNameTag()+" </font></span>");
	  }
...
function wordClick(nameID){
     console.log("wordclick");
     $('#'+nameID+'').css("background-color","yellow");      
}
