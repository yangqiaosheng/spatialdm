	...
	*/
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (MapUtils.isEmpty(request.getParameterMap())) {
			response.sendRedirect("RequestKmlDemo_milan.jsp");
			return;
		}

		// web base path for local operation
		String localBasePath = System.getProperty("prediction_milan.root");
		...
		if (StringUtils.isEmpty(xml)) {
			...
			try {
				...
				CsvImporter.importAreaEvents(areaEvents, localBasePath + "data/places_presence_predicted_2011_09_19-25.csv");

				List<Area> areas = Lists.newArrayList();
				CsvImporter.importAreas(areas, areaDto.getBoundaryRect(), localBasePath + "data/areas1000.xml");
