...
public class ModelManager {
	...
	public static void setTransfromVector(AreaDto areaDto) {
		double scale = 1.0;
		Point2D from = new Point2D.Double(9.1237, 45.478158);
	...
	public static Map<Long, Map<String, Integer>> generateEvents(AreaDto areaDto) throws ParseException {
		SimpleDateFormat areaDtoFormat = new SimpleDateFormat(minuteDateFormatStr);
	...
	public static String buildKmlFile(List<AreaResult> areaResults, String filenamePrefix, String radius, Point2D transfromVector, String remoteBasePath, boolean compress) throws UnsupportedEncodingException {
		String localBasePath = ModelManager.class.getResource("/../../").getPath();
		if (StringUtils.isEmpty(remoteBasePath)) {
			remoteBasePath = "http://localhost:8080/ESS/";
		}
...
//			polygonElement.addContent(new Element("altitudeMode", namespace).addContent("absolute"));
	...
	private static String buildKmlDescription(AreaResult areaResult) {
		...
		String minuteChartImg = "";
		if (MapUtils.isNotEmpty(areaResult.getHistograms().getMinutes())) {
			minuteChartImg = createChartImg("Photos Distribution", width / 2, 160, areaResult.getHistograms().getMinutes(), Level.MINUTE);
		}
		String description = weekdayChartImg + yearChartImg + "<BR>" + monthChartImg + "<BR>" + dayChartImg + "<BR>" + hourChartImg + "<BR>" + minuteChartImg;
		return description;
	}
...
//			}
	...
	public static void calculateHistograms(List<AreaResult> areaResults, AreaDto areaDto) {
		...
		for (AreaResult areaResult : areaResults) {
			...
			Map<Integer, Integer> minuteData = areaResult.getHistograms().getMinutes();
			Map<Integer, Integer> weekdayData = areaResult.getHistograms().getWeekdays();

			for (int minute : DateUtil.allMinuteInts(areaDto.getMinuteInterval())) {
				minuteData.put(minute, 0);
			}
			...
			for (Map.Entry<String, Integer> e : areaResult.getArea().getDatesCount().entrySet()) {
				if (areaDto.getQueryStrs().contains(e.getKey().substring(0, queryStrsLength))) {
					if (areaDto.getQueryLevel() == Level.MINUTE) {
						int minute = Integer.parseInt(e.getKey().substring(14, 16));
						minuteData.put(minute, e.getValue() + minuteData.get(minute));
					}
