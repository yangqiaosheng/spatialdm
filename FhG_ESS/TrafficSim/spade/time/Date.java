...
public class Date implements TimeMoment, java.io.Serializable {
  ...
  */
  public static boolean isTimeSymbol (char symbol) {
    ...
    for (int i=0; i<time_symbols.length; i++)
      if (symbol==time_symbols[i]) return true;
    return false;
  }
  /**
  ...
  public static boolean isSimple (String scheme) {
    ...
    if (!isTimeSymbol(first)) return false;
    for (int i=1; i<scheme.length(); i++)
      if (scheme.charAt(i)!=first) return false;
    return true;
  }
  /**
  ...
  */
    ...
    for (int i=0; i<time_symbols.length; i++)
      if (hasDateElement[i]) {
        switch (time_symbols[i]) {
          ...
          case 't': min=0; break;
          case 'h': hour=0; break;
          case 'd': day=(hasElement('m'))?1:0; break;
          case 'm': month=(hasElement('y'))?1:0; break;
        ...
        switch (time_symbols[i]) {
          ...
          case 'd':
            ...
            if (!ok) {
              ...
              if (hasElement('m')) err+=" in month "+month;
              return false;
            }
            break;
          case 'm':
  ...
  public char[] getAvailableTimeElements () {
    int len=0;
    for (int i=0; i<time_symbols.length; i++)
      if (hasDateElement[i]) ++len;
    if (len<1) return null;
    char elements[]=new char[len];
    len=0;
    for (int i=time_symbols.length-1; i>=0; i--)
      if (hasDateElement[i]) elements[len++]=time_symbols[i];
    return elements;
  }
  /**
  ...
  */
  ...
  public void setDate (GregorianCalendar gc) {
    ...
    if (requiresElement('d'))
      setDay(gc.get(Calendar.DAY_OF_MONTH));
    if (requiresElement('h'))
      setHour(gc.get(Calendar.HOUR_OF_DAY));
    if (requiresElement('t'))
      setMinute(gc.get(Calendar.MINUTE));
    if (requiresElement('s'))
      setSecond(gc.get(Calendar.SECOND));
    numVal=-1;
  }
  /**
  ...
  */
  public boolean setMoment (TimeMoment moment) {
    if (moment==null || !(moment instanceof Date)) return false;
    Date d=(Date)moment;
    if (d.hasElement('y')) setYear(d.year);
    if (d.hasElement('m')) setMonth(d.month);
    if (d.hasElement('d')) setDay(d.day);
    if (d.hasElement('h')) setHour(d.hour);
    if (d.hasElement('t')) setMinute(d.min);
    if (d.hasElement('s')) setSecond(d.sec);
    numVal=-1;
    return isSet();
  }
  /**
  ...
  */
  public boolean setMoment (String timeStr) {
    if (timeStr==null || scheme==null) return false;
    int i=0;
    ...
    while (i<minLength) {
      ...
      if (ch=='s' || ch=='t' || ch=='h' || ch=='d' || ch=='m' || ch=='y') {
        int len=1;
        for (int j=i+1; j<minLength && scheme.charAt(j)==ch; j++) ++len;
        ...
        if (num>=0)
          if (ch=='s') setSecond(num); else
          if (ch=='t') setMinute(num); else
          if (ch=='h') setHour(num); else
          if (ch=='d') setDay(num); else
          if (ch=='m') setMonth(num); else
          if (ch=='y') {
            if (len==2 && num<100)
              if (num>20) num+=1900;
              else num+=2000;
  ...
  */
  ...
  public void checkPrecision () {
    if (precIdx>=0 && hasDateElement[precIdx])
      return;
    while (precIdx>0 && !hasDateElement[precIdx]) --precIdx;
  ...
  */
  public boolean useElement (char elem) {
    ...
    int idx=getTimeUnitIdx(elem);
    if (idx<0) return false;
    if (!hasDateElement[idx]) return false;
    return precIdx>=idx;
  }
  /**
  ...
  */
  ...
  public static String removeExtraElementsFromScheme(String origScheme, char prec) {
    if (origScheme==null) return null;
  ...
  */
  public void setElementValue (char elem, int value) {
    switch (elem) {
      case 's': setSecond(value); break;
      case 't': setMinute(value); break;
      case 'h': setHour(value); break;
      case 'd': setDay(value); break;
      case 'm': setMonth(value); break;
      case 'y': setYear(value);
    }
    numVal=-1;
  }
  /**
  ...
  */
  public String getUnit () {
    if (useElement('s')) return res.getString("sec_");
    if (useElement('t')) return res.getString("min_");
    if (useElement('h')) return res.getString("hour");
    if (useElement('d')) return res.getString("day");
    if (useElement('m')) return res.getString("month");
    if (useElement('y')) return res.getString("year");
    return null;
  }
  /**
  ...
  */
  public String getUnits () {
    if (useElement('s')) return res.getString("sec_");
    if (useElement('t')) return res.getString("min_");
    if (useElement('h')) return res.getString("hours");
    if (useElement('d')) return res.getString("days");
    if (useElement('m')) return res.getString("months");
    if (useElement('y')) return res.getString("years");
    return null;
  }
  /**
  ...
  */
  public boolean isSet () {
    for (int i=0; i<hasDateElement.length; i++)
      if (hasDateElement[i]) return true;
    return false;
  }
  /**
  ...
  */
  public void setDefaultScheme () {
    ...
    if (useElement('d')) {
      sb.append("dd");
      if (useElement('m')) sb.append(".");
    }
    if (useElement('m')) {
      sb.append("mm");
      if (useElement('y')) sb.append(".");
    }
    if (useElement('y')) sb.append("yyyy");
    if (sb.length()>0 && (useElement('h') || useElement('t') || useElement('s')))
      sb.append("; ");
    if (useElement('h')) {
      sb.append("hh");
      if (useElement('t')) sb.append(":"); else sb.append("H");
    }
    if (useElement('t')) {
      sb.append("tt");
      if (useElement('s')) sb.append(":");
      else
        if (!useElement('h')) sb.append("\'");
    }
    if (useElement('s')) {
      sb.append("ss");
      if (!useElement('h')) sb.append("\"");
  ...
  */
  public String toString () {
    ...
    if (scheme==null) setDefaultScheme();
    int i=0;
    ...
    while (i<scheme.length()) {
      ...
      if (ch=='s' || ch=='t' || ch=='h' || ch=='d' || ch=='m' || ch=='y') {
        int len=1;
        for (int j=i+1; j<scheme.length() && scheme.charAt(j)==ch; j++) ++len;
        i+=len;
        int num=sec;
        if (ch=='t') num=min; else
        if (ch=='h') num=hour; else
        if (ch=='d') num=day; else
        if (ch=='m') num=month; else
        if (ch=='y') num=year;
        if (num<0) num=0;
        ...
        if (str.length()>len) str=str.substring(str.length()-len);
          ...
          if (str.length()<len)
            for (int j=str.length(); j<len; j++) sb.append('0');
        sb.append(str);
      }
      else
  ...
  public String toSQLstring() {
    ...
    r[1]="ddmmyyyy";
    r[0]=StringUtil.padString(""+Math.max(1,day),'0',2,true)+
         StringUtil.padString(""+Math.max(1,month),'0',2,true)+
         StringUtil.padString(""+Math.max(1,year),'0',4,true);
  ...
  */
  public TimeMoment copyTo (TimeMoment t) {
    ...
    d.scheme=scheme;
    d.precIdx=precIdx;
    d.year=year; d.month=month; d.day=day; d.hour=hour; d.min=min; d.sec=sec;
    for (int i=0; i<hasDateElement.length; i++)
  ...
  */
  public void stepForth () {
    ...
    if (useElement('s')) {
      ++sec;
      if (sec>=60 && hasElement('t')) sec=0; else return;
    }
    if (useElement('t')) {
      ++min;
      if (min>=60 && hasElement('h')) min=0; else return;
    }
    if (useElement('h')) {
      ++hour;
      if (hour>=24 && hasElement('d')) hour=0; else return;
    }
    if (useElement('d')) {
      ++day;
      if (!hasElement('m') || day<=28) return;
      switch (month) {
    ...
    if (useElement('m')) {
      ++month;
      if (month>12 && hasElement('y')) month=1; else return;
    }
    ++year;
  }
  /**
  ...
  */
  public void stepBack () {
    ...
    if (useElement('s')) {
      --sec;
      if (sec<0 && hasElement('t')) sec=59; else return;
    }
    if (useElement('t')) {
      --min;
      if (min<0 && hasElement('h')) min=59; else return;
    }
    if (useElement('h')) {
      --hour;
      if (hour<0 && hasElement('d')) hour=23; else return;
    }
    if (useElement('d')) {
      --day;
      if (day>0 || !hasElement('m')) return;
      switch (month-1) {
        ...
        case 2:
          day=28;
          if (hasElement('y'))
            if (nDaysInYear(year)==366) day=29;
          break;
        default:
          day=30;
      }
    ...
    if (useElement('m')) {
      --month;
      if (month<1 && hasElement('y')) month=12; else return;
    }
    --year;
  }
  /**
  ...
  */
  public boolean equals (Object t) {
    if (t==null || !(t instanceof Date)) return false;
    Date d=(Date)t;
    if (useElement('y') && year!=d.year) return false;
    if (useElement('m') && month!=d.month) return false;
    if (useElement('d') && day!=d.day) return false;
    if (useElement('h') && hour!=d.hour) return false;
    if (useElement('t') && min!=d.min) return false;
    if (useElement('s') && sec!=d.sec) return false;
    return true;
  }
  /**
  ...
  */
  public int compareTo (Comparable t) {
    ...
    Date d=(Date)t;
    if (useElement('y') && d.hasElement('y'))
      if (year<d.year) return -1; else
      if (year>d.year) return 1;
    if (useElement('m') && d.hasElement('m'))
      if (month<d.month) return -1; else
      if (month>d.month) return 1;
    if (useElement('d') && d.hasElement('d'))
      if (day<d.day) return -1; else
      if (day>d.day) return 1;
    if (useElement('h') && d.hasElement('h'))
      if (hour<d.hour) return -1; else
      if (hour>d.hour) return 1;
    if (useElement('t') && d.hasElement('t'))
      if (min<d.min) return -1; else
      if (min>d.min) return 1;
    if (useElement('s') && d.hasElement('s'))
      if (sec<d.sec) return -1; else
  ...
  public static int nDaysInYear(int year){
    if (year<=0) return 0;
    if (year%4==0 && (year%100!=0 || (year/100)%4==0)) return 366;
    return 365;
  }
    ...
    for (int i=1; i<month; i++)
      switch (i) {
        case 1: case 3: case 5: case 7: case 8: case 10: case 12:
          n+=31; break;
        case 2:
          if (nDaysInYear(year)==366) n+=29; else n+=28; break;
        default:
          n+=30;
      }
    return n+day;
  ...
  public static int nDaysInMonth(int month,int year) {
    switch (month) {
      case 1: case 3: case 5: case 7: case 8: case 10: case 12:
        return 31;
      case 2: {
        if (nDaysInYear(year)==366) return 29;
        return 28;
      }
    }
    return 30;
  ...
  */
  public long subtract(TimeMoment t) {
    if (t==null || !(t instanceof Date)) return 0l;
    Date d=(Date)t;
    int precIdx1=this.getPrecisionIdx();
    int precIdxH1=this.getHighestPossiblePrecisionIdx(),
        precIdxH2=d.getHighestPossiblePrecisionIdx();
  ...
  */
  public synchronized long toNumber () {
    ...
    if (useElement('h')) val=val*24+hour;
    if (useElement('t')) val=val*60+min;
    if (useElement('s')) val=val*60+sec;
    numVal=val;
    return val;
  }
  /**
  ...
  */
  public synchronized long toNumber (int precIdx) {
    ...
    if (precIdx>=3 && hasDateElement[3]) val=val*24+hour;
    if (precIdx>=4 && hasDateElement[4]) val=val*60+min;
    if (precIdx>=5 && hasDateElement[5]) val=val*60+sec;
    numVal=val;
    return val;
  }
  /**
  ...
  */
  public TimeMoment valueOf (long n)  {
    ...
    d.reset();
    long val=n;
    if (useElement('s')) { d.setElementValue('s',(int)(val%60)); val/=60; }
    if (useElement('t')) { d.setElementValue('t',(int)(val%60)); val/=60; }
    if (useElement('h')) { d.setElementValue('h',(int)(val%24)); val/=24; }
    if (useElement('d')) {
      if (!hasElement('m')) { d.day=(int)val; return d; }
      if (hasElement('y')) {
    ...
    if (useElement('m')) {
      ...
      d.setElementValue('m',(int)(val%12)); val/=12;
    }
    if (useElement('y')) d.setElementValue('y',(int)val);
    return d;
  }
  /**
  ...
  */
  public void add (long nUnits){
    if (nUnits==0) return;
    if (nUnits>0)
      for (int i=0; i<nUnits; i++) stepForth();
    else
      for (int i=0; i<-nUnits; i++) stepBack();
  }
  /*
  ...
  */
  ...
  public static String checkTemplateValidity (String template) {
    ...
    if (idx<0)
      return res.getString("no_time_symbols")+" (y, m, d, h, t, s) "+
             inTemplText+"!";
    ...
    for (int i=0; i<time_symbols.length; i++) {
      ...
      if (template.lastIndexOf(ch)>idx) {
        return res.getString("Symbols")+" \""+String.valueOf(ch)+"\" "+
               res.getString("must_come_in_sequence")+" "+
               inTemplText+"!";
      }
    }
    for (int i=0; i<used.length-2; i++)
      if (used[i] && !used[i+1]) {
        ...
        for (int j=i+2; j<used.length && next<0; j++)
          if (used[j]) next=j;
        if (next<0) continue;
        String err=res.getString("Symbols")+" \""+time_symbols[i]+"\" "+
                   res.getString("and")+" \""+time_symbols[next]+"\" "+
                   res.getString("but_no")+" ";
  ...
  public static String getTextForTimeSymbol (char symb) {
    switch (symb) {
      case 'y': case 'Y': return res.getString("year");
      case 'm': case 'M': return res.getString("month");
      case 'd': case 'D': return res.getString("day");
      case 'h': case 'H': return res.getString("hour");
      case 't': case 'T': return res.getString("minute");
      case 's': case 'S': return res.getString("second");
    }
    return null;
  }
  /**
  ...
  public static String getTextForTimeSymbolInPlural (char symb) {
    switch (symb) {
      case 'y': case 'Y': return res.getString("years");
      case 'm': case 'M': return res.getString("months");
      case 'd': case 'D': return res.getString("days");
      case 'h': case 'H': return res.getString("hours");
      case 't': case 'T': return res.getString("minutes");
      case 's': case 'S': return res.getString("seconds");
    }
    return null;
  }
  /**
  ...
  public void roundDown () {
    ...
    if (precision==getMinPrecision()) return; //no need to round
    numVal=-1;
    if (hasElement('s') && sec>0)
      sec=0;
    if (precision=='t') return;
    if (hasElement('t') && min>0)
      min=0;
    if (precision=='h') return;
    if (hasElement('h') && hour>0)
      hour=0;
    if (precision=='d') return;
    if (hasElement('d') && day>1)
      day=1;
    if (precision=='m') return;
  ...
  public boolean isNice() {
    ...
    if (precIdx>=3) return val==0;
    if (precIdx>0) return val==1;
    return val%10==0;
  }
  /**
   * Transforms the given duration (time interval length) from the
   * original time units to the desired time units
   */
  public static long transformDuration(long duration, char origPrec, char prec) {
    if (origPrec==prec || duration<=0)
      return duration;
    int idx0=getTimeUnitIdx(prec), idx1=getTimeUnitIdx(origPrec);
    while (idx1>idx0) {
      switch (idx1) {
        case 5: case 4: //seconds, minutes
        ...
        case 2: //days
          if (idx0==1)
            duration=Math.round(duration/30.0);
          else {
            duration=Math.round(duration/365.0);
            --idx1;
          }
          break;
      }
