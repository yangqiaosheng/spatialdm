...
public class StringUtil {
  ...
  */
  public static boolean isStringInVectorIgnoreCase(String str, Vector v){
    if (v==null || v.size()<1 || str==null) return false;
    for (int i=0; i<v.size(); i++)
      if (v.elementAt(i)!=null && v.elementAt(i) instanceof String)
        if (str.equalsIgnoreCase((String)v.elementAt(i))) return true;
    return false;
  }
  /**
  ...
  */
  public static int indexOfStringInVectorIgnoreCase(String str, Vector v){
    if (v==null || v.size()<1 || str==null) return -1;
    for (int i=0; i<v.size(); i++)
      if (v.elementAt(i)!=null && v.elementAt(i) instanceof String)
        if (str.equalsIgnoreCase((String)v.elementAt(i))) return i;
    return -1;
  }
  /**
  ...
  */
  public static boolean isSubsetOf (Vector subset, Vector superset) {
    ...
    if (subset.size()<1 || superset.size()<1) return false;
  ...
  */
  public static boolean isSubsetOf (Vector subset, Vector superset, int startIdx) {
    if (subset==null || superset==null) return false;
    if (subset.size()<1 || superset.size()<=startIdx) return false;
    for (int i=0; i<subset.size(); i++)
  ...
  */
  public static boolean Larger (String st1, String st2) {
    ...
    for (int i=0; i<maxl; i++) {
      if (i>st1.length()-1) return false;
      if (i>st2.length()-1) return true;
      if (st1.charAt(i)==st2.charAt(i)) continue;
  ...
  public static Vector getNames(String nameStr) {
    ...
    if (nameStr.startsWith("\"")) {
      ...
      while (i1>=0 && i1<nameStr.length()) {
        ...
        if (i2<0) i2=nameStr.length();
        if (i2>i1+1) {
          String s=nameStr.substring(i1+1,i2).trim();
          if (s.length()>0) {
            if (names==null) names=new Vector(10,10);
            names.addElement(s);
          }
        }
        if (i2<nameStr.length()-1) i1=nameStr.indexOf('\"',i2+1);
        else i1=-1;
      }
    }
    else {
    ...
    while (nameStr!=null && nameStr.length()>0) {
      if (delimiter!=null && delimiter.length()>0) {
        ...
        while (delimiter.indexOf(nameStr.charAt(i))>=0) {
          ++i;
          if (retrieveEmptyNames) names.addElement("");
        }
        if (i>0) nameStr=nameStr.substring(i);
        nameStr=nameStr.trim();
        if (nameStr.length()<1) break;
      }
      if (nameStr.startsWith("\"")) {
        ...
        while (i2<nameStr.length() && delimiter.indexOf(nameStr.charAt(i2))<0)
          ++i2;
        if (i2>=nameStr.length()) nameStr=null;
        else nameStr=nameStr.substring(i2+1);
      }
      else {
        ...
        while (i2<nameStr.length() && delimiter.indexOf(nameStr.charAt(i2))<0)
          ++i2;
        if (i2>0) names.addElement(nameStr.substring(0,i2).trim());
        else names.addElement("");
        if (i2>=nameStr.length()) nameStr=null;
        else nameStr=nameStr.substring(i2+1);
      }
    }
  ...
  */
  ...
  public static String padString(String str, char ch, int len, boolean left) {
    ...
    if (left) return sb.toString()+str;
  ...
  public static String dateToString(Calendar date, String separator) {
    ...
    return padString(String.valueOf(date.get(Calendar.DAY_OF_MONTH)),'0',2,true)+separator+
        padString(String.valueOf(date.get(Calendar.MONTH)+1),'0',2,true)+separator+
  ...
  public static String timeToString (Calendar date, String separator,
    ...
    if (date==null) return null;
    String str=padString(String.valueOf(date.get(Calendar.HOUR_OF_DAY)),'0',2,true)+
        separator+
        padString(String.valueOf(date.get(Calendar.MINUTE)),'0',2,true);
    if (includeSeconds)
      str+=separator+
          padString(String.valueOf(date.get(Calendar.SECOND)),'0',2,true);
    return str;
  }
  /**
   * Returns a string representation of the time of the day specified in the
  ...
  */
  public static String floatToStr (float fl, int prec) {
    if (fl>=Long.MAX_VALUE || fl<=Long.MIN_VALUE) return String.valueOf(fl);
    long factor=1;
  ...
  */
  public static int getPreferredPrecision (float fl, float min, float max) {
    if (Float.isNaN(fl)) return 0;
    if (Float.isNaN(min)) min=fl;
    if (Float.isNaN(max)) max=fl;
    float dif=(max>min)?max-min:max;
    int prec=0;
    while (dif>0 && dif<1000.0f) {dif*=10; ++prec; }
    return prec;
  }
  /**
  ...
  */
  public static String floatToStr (float fl, float min, float max) {
    if (Float.isNaN(fl)) return String.valueOf(fl);
  ...
  */
  public static int getPreferredPrecision (double fl, double min, double max) {
    if (Double.isNaN(fl)) return 0;
    if (Double.isNaN(min)) min=fl;
    if (Double.isNaN(max)) max=fl;
  ...
  */
  public static String doubleToStr (double fl, double min, double max) {
    if (Double.isNaN(fl)) return String.valueOf(fl);
  ...
  */
  public static String replace (String txt, String oldS, String newS) {
    if (txt==null || oldS==null || oldS.equals(newS)) return txt;
    if (txt.indexOf(oldS)<0) return txt;
    int i=0;
    String result="";
    while (i<txt.length()) {
      int j=txt.indexOf(oldS,i);
      if (j<0) j=txt.length();
      if (j>i) result+=txt.substring(i,j);
      if (j<txt.length()) {
        result+=newS;
        i=j+oldS.length();
      }
  ...
  */
   ...
   */
    public static String SQLid(String s, int length) {
      ...
      if (ss.charAt(0) == '_') ss = "x" + ss.substring(1);
      try { new Integer(ss); ss = "x" + ss;}
      catch (Exception ex) {}
   ...
   */
    ...
    */
    public static String modifyId(String s, int maxlen) {
      if (maxlen <= 0) return modifyId(s, true);
      if (s.length() <= maxlen-2) return modifyId(s, true);
      else return modifyId(s, false);
    }
//~ID
