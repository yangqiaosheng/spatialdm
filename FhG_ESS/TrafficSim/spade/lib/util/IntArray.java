...
public class IntArray implements java.io.Serializable{
  ...
  public IntArray(){ initialize(10,10);}
  ...
  public void initialize(int initialCapacity, int increment) {
    incr=increment; nelements=0;
    if (initialCapacity<5) initialCapacity=5;
    if (incr<1) incr=1;
  ...
  protected void increaseCapacity(){
    ...
    if (nelements>numbers.length) nelements=numbers.length;
    for (int i=0; i<nelements; i++) ext[i]=numbers[i];
    numbers=ext;
  }
  public void addElement(int element) {
    if (nelements+1>numbers.length) increaseCapacity();
  ...
  public int elementAt(int idx){
    if (idx<0 || idx>=nelements) return Integer.MIN_VALUE;
    return numbers[idx];
  }
  ...
  public int[] getArray() { return numbers; }
  ...
  public Object clone () {
    int count=nelements;
    if (numbers!=null && count<numbers.length) count=numbers.length;
    if (count==0) count=10;
  ...
  public boolean contains (IntArray anotherArray) {
    if (anotherArray==null || anotherArray.size()<1) return true;
    if (nelements<1) return false;
    for (int i=0; i<anotherArray.size(); i++)
      if (indexOf(anotherArray.elementAt(i))<0) return false;
    return true;
  }
  /**
